...
$_echosign_api_url = (string) 'https://secure.echosign.com/services/EchoSignDocumentxxxxxx?wsdl';
$_echosign_api_key = (string) "apikeyxxxxxx";
...
// load the data from the spreadsheet file
...
foreach ($worksheetNames as $key => $sheetName) {
     ...
     *
     * $nullValue Value returned in the array entry if a cell doesn't exist
     * $calculateFormulas Should formulas be calculated?
     * $formatData Should formatting be applied to cell values?
     * $returnCellRef False - Return a simple array of rows and columns indexed by number counting from zero
...
// loop through the emails array and get the doc_key from the database
$k = 0;
...
try {
    ...
    // loop through the dockey array and get the contents for the documents for the selected doc_key from echosign
    foreach ($dockey_array as $key => $doc_key) {
        $doc_result = '';

        // get the documents contents by dockey
        $doc_result = $S->getDocuments(array(
            'apiKey' => $_echosign_api_key,
            'documentKey' => $doc_key,
            'options' => true
        ));
